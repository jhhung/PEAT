...
//		return ;
	...
	void Trim (std::map < int, std::vector< FORMAT<TUPLETYPE> > >* result2, int q)
	{
//		return ;
		this->check_read_length (result2);
		auto job_count = std::min ( this->parameter_trait.num, std::min ( (*result2)[0].size(), (*result2)[1].size() ) );
...
//			size_t dif = job_count/this->parameter_trait.pool_size;
			...
			jd.distribute_jobs( *result2, *result2, this->parameter_trait.pool_size,
			[this] (std::map <int, std::vector< FORMAT<TUPLETYPE> > >& result2, 
					std::shared_ptr < std::map < int, std::vector<size_t> > > trim_position, 
					size_t front, size_t job_count)
			{
				this->TrimImpl (result2, (*trim_position)[0], front, job_count);
			}
//			for (size_t i=0; i!=this->parameter_trait.pool_size; ++i)
//			{
//				index_vec.push_back ( 
